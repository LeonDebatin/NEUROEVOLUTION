            ...
            if random.random() < prob:
                if isinstance(node, _Function):
                    ...
                    if random.random() < sspace["p_constants"]:
                        #repr_copy[i] = sspace["constant_set"][random.randint(0, len(sspace["constant_set"]) - 1)] fixed
                        #adaption
                        repr_copy[i] = torch.tensor(random.uniform(sspace['constant_set'].min, sspace['constant_set'].max))
                    else:
