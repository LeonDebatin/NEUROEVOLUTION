...
def ne_cross_validation(X, y, batch_size, shuffle, kfold, initializer, ps, n_iter,  sspace,  selection_method, mutation_prob, mutation_method, xo_prob, xo_method, has_elitism, allow_reproduction,log_path_cv, log_path_train, ffunction, seed,  device, id=None):
    ...
    for fold, (train_index, val_index) in enumerate(kfold.split(X)):
        ...
        m = ne_train(dl_train = dl_train, dl_val = dl_val, initializer=initializer, ps=ps, n_iter=n_iter, sspace = sspace, selection_method=selection_method, mutation_method=mutation_method, xo_method=xo_method, ffunction=ffunction, log_path= log_path_train,  mutation_prob=mutation_prob, xo_prob=xo_prob, has_elitism=has_elitism, allow_reproduction=allow_reproduction,  seed=seed,  device=device)
